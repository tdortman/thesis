...
Recent work by McCoy et al. introduced the Two-Choice Filter (TCF), a data structure designed specifically for high-throughput, parallel execution on GPUs \cite{tcf}. The TCF shares the same high-level goal as the GPU-accelerated Cuckoo filter presented in this thesis: to provide a deletable, space-efficient filter optimized for the architectural constraints of a GPU.
...
This approach guarantees a fixed number of memory accesses per insertion, preserving memory coherence. However, because the blocks must be small to fit in GPU cache lines, the statistical variance in load distribution increases, leading to premature failures even when the total table is not full. To address this, the TCF relies on a backing store, a small, secondary hash table to catch these overflows. This hybrid architecture allows the TCF to maintain a high overall occupancy (up to 95\%) while keeping the primary insertion logic simple and coherent.
