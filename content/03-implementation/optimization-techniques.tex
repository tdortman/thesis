...
\begin{itemize}
  \item If the choice bit is 0, the item is in its primary bucket $b$. The alternate bucket is calculated as:
    \[
      b' = (b + \text{offset}(fp)) \bmod m
    \]
  \item If the choice bit is 1, the item is in its alternate bucket $b'$. The primary bucket is calculated as:
    \[
      b = (b' - \text{offset}(fp)) \bmod m
    \]
\end{itemize}
