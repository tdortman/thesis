...
The design of the GPU-accelerated Cuckoo filter emphasizes compile-time configuration and a memory layout optimized for cache-friendly access patterns.
...
To maximize performance by allowing the compiler to generate highly specialized code, the filter's core parameters are defined at compile-time as template parameters.
...
\begin{itemize}
  ...
  \item \texttt{blockSize}: Configures the thread block size for the internal CUDA kernels.
    This parameter can be tuned to optimize GPU occupancy for different hardware architectures.
  ...
  \item \texttt{evictionPolicy}: The policy used to handle insertions into full buckets.
    ...
    This strategy minimizes the length of eviction chains and reduces global memory traffic (See Sections \ref{sec:implementation:optimisation-techniques:eviction} \& \ref{sec:eval:eviction-policies}).
...
To maximize memory bandwidth and avoid misaligned access, the internal layout is carefully structured:
...
Figure \ref{fig:memory-layout} visualizes this hierarchical memory layout.
...
Its value is only lazily copied to the host when an explicit query is made, minimizing host-device communication.
