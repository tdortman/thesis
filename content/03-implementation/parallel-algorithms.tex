  ...
  \begin{algorithmic}[1]
    \Function{Insert}{\texttt{key}}
      ...
      \For{\texttt{n} $= 1$ \textbf{to} \texttt{maxEvictions}}
        ...
        \State \texttt{tag} $\gets$ \Call{AtomicExchange}{\texttt{bucket}[\texttt{b}].\texttt{slot}[\texttt{s}], \texttt{tag}}
        \State \texttt{b} $\gets$ \Call{AlternateIndex}{\texttt{b}, \texttt{tag}}

        \If{\Call{TryInsert}{\texttt{bucket}[\texttt{b}], \texttt{tag}}}
          \State \textbf{return} \texttt{Success}
        \EndIf
      \EndFor
   ...
   \Function{TryInsert}{bucket, tag}
      ...
      \For{\texttt{i} $= 0$ \textbf{to} \texttt{wordsPerBucket} $- 1$}
        ...
        \While{\texttt{mask} is not 0}
          ...
          \If{\Call{AtomicCAS}{\&\texttt{slot}, \texttt{EMPTY}, \texttt{tag}}}
            \State \textbf{return} \texttt{Success}
          \EndIf
...
\begin{algorithm}[htbp]
  ...
  \begin{algorithmic}[1]
    \Function{Contains}{\texttt{key}}
      ...
      \State \textbf{return} \Call{Find}{$i_1$, \texttt{fp}} \textbf{or} \Call{Find}{$i_2$, \texttt{fp}}
    \EndFunction
    \Statex
...
\begin{algorithm}[htbp]
  ...
  \begin{algorithmic}[1]
    \Function{Remove}{\texttt{key}}
      ...
      \State \textbf{return} \Call{TryRemove}{$i_1$, \texttt{fp}} \textbf{or} \Call{TryRemove}{$i_2$, \texttt{fp}}
    \EndFunction
    \Statex
    \Function{TryRemove}{\texttt{bucket}, \texttt{targetTag}}
      ...
      \For{\texttt{i} $= 0$ \textbf{to} \texttt{wordsPerBucket} $- 1$}
        ...
        \While{\texttt{mask} is not 0}
           ...
           \If{\Call{AtomicCAS}{\&\texttt{slot}, \texttt{targetTag}, \texttt{EMPTY}}}
             \State \textbf{return} \texttt{Success}
           \EndIf
