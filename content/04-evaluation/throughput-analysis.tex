...
In this scenario, the filter size (approx. 4.2 million items) is small enough to fit entirely within the GPU's L2 cache. This isolates the algorithmic efficiency and instruction throughput from main memory latency. The following figures display the performance on System A (GDDR7) on the left and System B (HBM3) on the right.

\subsubsection{Insertion Performance}
Figure \ref{fig:insert-small} illustrates the insertion throughput. The results demonstrate that the GPU Cuckoo filter is highly competitive, bridging the gap between static and dynamic data structures.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=1.0\textwidth]{images/perf/insert_small.png}
  \caption{Insert Performance on System A (Left) and System B (Right) for an L2-resident filter}
  \label{fig:insert-small}
\end{figure}

\FloatBarrier

\subsubsection{Lookup Performance}
The query performance is evaluated by measuring throughput for both positive lookups (keys present in the filter) and negative lookups (keys absent). The results are shown in Figure \ref{fig:query-small}.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=1.0\textwidth]{images/perf/query_small.png}
  \caption{Query Performance on System A (Left) and System B (Right) for an L2-resident filter.}
  \label{fig:query-small}
\end{figure}

\FloatBarrier

\subsubsection{Deletion Performance}
Deletion performance, illustrated in Figure \ref{fig:delete-small}, highlights the most significant performance advantage of the GPU Cuckoo filter over its competitors.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=1.0\textwidth]{images/perf/delete_small.png}
  \caption{Deletion Performance on System A (Left) and System B (Right) for an L2-resident filter.}
  \label{fig:delete-small}
\end{figure}
...
The insertion throughput for the DRAM-resident workload is illustrated in Figure \ref{fig:insert-large}.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=1.0\textwidth]{images/perf/insert_large.png}
  \caption{Insertion Performance on System A (Left) and System B (Right) for a DRAM-resident filter.}
  \label{fig:insert-large}
\end{figure}

\FloatBarrier

\subsubsection{Lookup Performance}
Query performance is presented in Figure \ref{fig:query-large}. Across both systems, the Cuckoo filter maintains a distinct performance profile based on the query result:
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=1.0\textwidth]{images/perf/query_large.png}
  \caption{Query Performance on System A (Left) and System B (Right) for a DRAM-resident filter.}
  \label{fig:query-large}
\end{figure}

\FloatBarrier

\subsubsection{Deletion Performance}
Figure \ref{fig:delete-large} illustrates the deletion performance.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=1.0\textwidth]{images/perf/delete_large.png}
  \caption{Deletion Performance on System A (Left) and System B (Right) for a DRAM-resident filter.}
  \label{fig:delete-large}
\end{figure}
