...
\begin{itemize}
  ...
  \item \textbf{Large Buckets}: Excessively large buckets introduce hardware-level inefficiencies. If a bucket's size exceeds the GPU's cache line size, fetching a single logical bucket requires multiple physical memory transactions. In the worst case this doubles the number of cache lines that must be fetched, effectively halving the memory bandwidth.
\end{itemize}
