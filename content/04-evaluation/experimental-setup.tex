...
\begin{itemize}
  \item \textbf{System A (GDDR6X)}: An AMD Ryzen Threadripper 3990X (64 cores) paired with an NVIDIA RTX 4090 GPU featuring 24 GB of GDDR6X memory (1 TB/s). The system runs AlmaLinux 10.1 with NVIDIA driver 580.95.05 and CUDA 12.9.86.
...
It is important to note that these systems are not compared head-to-head to determine which is "better." Rather, they are used to evaluate how the algorithms respond to high-bandwidth memory (HBM) versus standard graphics memory (GDDR) while keeping as many other factors as possible constant.
...
\begin{itemize}
  ...
  \item \textbf{Optimized Partitioned CPU Cuckoo Filter}: A custom implementation utilizing multithreading (one thread per partition) to maximize CPU throughput without the overhead of SIMD instructions, which were found to degrade performance in this specific context \cite{partitioned-cuckoo}. Note that this filter was excluded from the System B (ARM-based) benchmarks, as its implementation has a hard dependency on x86-64 intrinsics that prevents compilation.
