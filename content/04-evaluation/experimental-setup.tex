...
\begin{itemize}
  ...
  \item \textbf{Optimized Partitioned CPU Cuckoo Filter}: A variant \cite{partitioned-cuckoo} utilizing multithreading (one thread per partition) to maximize CPU throughput without the overhead of SIMD instructions, which were found to degrade performance in this specific context. Note that this filter was excluded from the System B (ARM-based) benchmarks, as it has a hard dependency on x86-64 intrinsics that prevents compilation.
