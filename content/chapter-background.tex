...
For a given number of items $n$ and a filter size of $m$ bits, the optimal number of hash functions to minimize the false positive rate is given by $$k = \frac{m}{n} \ln 2$$
...
\begin{itemize}
  ...
  \item \textbf{Suboptimal Space Usage}: The optimal space usage for a Bloom filter is approximately $1.44 \log_2(1/\epsilon)$ bits per element for a desired false positive rate $\epsilon$, which is $44\%$ higher than the information theoretical lower bound.
\end{itemize}
